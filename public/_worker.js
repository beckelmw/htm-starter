...
__export(posts_exports, {
  default: () => Posts,
  getServerSideData: () => getServerSideData2
});
async function getServerSideData2({ env }) {
...
__export(id_exports, {
  default: () => Post,
  getServerSideData: () => getServerSideData3
});
async function getServerSideData3({ params }) {
  const { id } = params;
...
function Router(context) {
  ...
  for (const route of routes) {
    router.get(route.path, import_itty_router_extras.withParams, ({ params }) => render(HtmlPage, route.code, params));
  }
